    ...
    <div className="pt-24 pb-20 bg-white min-h-screen">
      ...
      <div className="container mx-auto px-4 md:px-8">
        {loading ? (
          ...
          <div className="text-center py-20 text-slate-500">No projects to display yet.</div>
        ) : (
          <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
            {projects.map((project) => (
              <div key={project.id} className="break-inside-avoid relative group overflow-hidden rounded-lg shadow-sm hover:shadow-xl transition-shadow cursor-pointer">
                <img 
                  src={project.image} 
                  alt={project.title} 
                ...
                <div className="absolute inset-0 bg-slate-900/0 group-hover:bg-slate-900/40 transition-colors duration-300 flex items-center justify-center">
                  <span className="text-white font-serif text-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform translate-y-4 group-hover:translate-y-0 text-center px-4">
                    {project.title}
                  </span>
                </div>
              </div>
